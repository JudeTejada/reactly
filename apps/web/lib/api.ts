...
class ApiClient {
  ...
  // Job-based insights generation
  async createInsightsJob(params: {
    projectId?: string;
    startDate?: string;
    endDate?: string;
  }): Promise<{
    ...
    const query = queryParams.toString();
    return this.fetch(`/insights/jobs${query ? `?${query}` : ""}`, {
      method: "POST",
    });
  }

  async getInsightsJobStatus(jobId: string): Promise<{
    status:
      | "pending"
      | "processing"
      | "completed"
    ...
    return this.fetch(`/insights/jobs/${jobId}`);
  }

  async cancelInsightsJob(jobId: string): Promise<void> {
    return this.fetch(`/insights/jobs/${jobId}`, {
      method: "DELETE",
    });
  }
