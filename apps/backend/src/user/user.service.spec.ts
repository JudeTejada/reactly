...
describe("UserService", () => {
  ...
  describe("getUserInternalId", () => {
    it("should return user internal ID when user exists", async () => {
      ...
      expect(mockGetUserInternalIdProvider).toHaveBeenCalledWith(
        "clerk-user-id"
      );
    });

    it("should throw NotFoundException when user does not exist", async () => {
      mockGetUserInternalIdProvider.mockRejectedValue(
        new NotFoundException("User not found")
      );

      await expect(
        service.getUserInternalId("nonexistent-user")
  ...
  describe("getUserByClerkId", () => {
    it("should return full user when user exists", async () => {
      ...
      expect(mockGetUserByClerkIdProvider).toHaveBeenCalledWith(
        "clerk-user-id"
      );
    });

    it("should throw NotFoundException when user does not exist", async () => {
      mockGetUserByClerkIdProvider.mockRejectedValue(
        new NotFoundException("User not found")
      );

      await expect(
        service.getUserByClerkId("nonexistent-user")
      ).rejects.toThrow(NotFoundException);
      expect(mockGetUserByClerkIdProvider).toHaveBeenCalledWith(
        "nonexistent-user"
      );
  ...
  describe("ownsProject", () => {
    it("should return true when user owns the project", async () => {
      ...
      expect(mockCheckProjectOwnershipProvider).toHaveBeenCalledWith(
        "user-uuid",
        "project-uuid"
      );
    });
  ...
  describe("upsertUserFromClerk", () => {
    ...
    it("should create new user when user does not exist", async () => {
      ...
      expect(mockUpsertUserFromClerkProvider).toHaveBeenCalledWith(
        mockClerkUser
      );
    });
  ...
  describe("deleteUser", () => {
    ...
    it("should throw NotFoundException when user does not exist", async () => {
      ...
      await expect(service.deleteUser("nonexistent-user")).rejects.toThrow(
        NotFoundException
      );
      expect(mockGetUserInternalIdProvider).toHaveBeenCalledWith(
        "nonexistent-user"
